...
std::vector<trajTuple> MotionPrimitive::operator()(const cspace::state_t x) {
  ...
  for (std::size_t i = 0; i < template_primitives_neutral.size(); i++) {
    ...
    primitives.push_back(std::make_tuple(std::get<0>(template_primitives_neutral[i]), traj,
                                         std::get<2>(template_primitives_neutral[i])));
  }
  ...
  for (std::size_t i = 0; i < speeds.size(); i++) {
    ...
    if (std::abs(-speeds[i] + x[3]) <= max_speed) {
      ...
      primitives.push_back(std::make_tuple(std::get<0>(template_primitives[i]), traj, neg_input));
    }
  }
  return primitives;
}
